...
class Episode0(Scene):
    ...
    def b05_cities(self):
        ...
        self.play(FadeIn(backing2), AddTextWordByWord(tally))
        self.wait(2)
    ...
    def b10_preferences(self):
        ...
        cakes = centered(tex_row(['Chocolate Cake', '$\\prec$', 'Carrot Cake']))
        self.play(FadeIn(cakes, lag_ratio=0.3))
        self.wait()
        self.play(FadeOut(cakes))

        beat(self, 'B11_coffee')
        # less preferred on the left, as on the number line to come
        light, lt, dark = Tex('Light Roast'), Tex('$\\prec$'), Tex('Dark Roast')
        pair = centered(VGroup(light, lt, dark).arrange(RIGHT, buff=0.25))
        self.play(FadeIn(pair, lag_ratio=0.3))
        self.wait()

        beat(self, 'B12_medium_inserts')
        # the pair opens in the middle and medium roast inserts
        mid = VGroup(Tex('Medium Roast'), Tex('$\\prec$')).arrange(RIGHT, buff=0.25)
        full = VGroup(light.copy(), lt.copy(), mid, dark.copy()).arrange(RIGHT, buff=0.25)
        centered(full)
        self.play(light.animate.move_to(full[0]), lt.animate.move_to(full[1]), dark.animate.move_to(full[3]))
        mid.move_to(full[2])
        self.play(FadeIn(mid, shift=0.2 * DOWN))
        self.wait()
        ...
        def swap_group(old_line, items):
            ...
            new_line.remove(new_line.line)          # reuse the line already on screen
            self.add(new_line)
            self.play(FadeIn(VGroup(*new_line.marks.values()), lag_ratio=0.1), run_time=0.8)
            new_line.line = old_line.line
        ...
        self.wait()
        V = swap_group(V, {'Decaf': 0.5, 'Light Roast': 2, 'Medium Roast': 5, 'Dark Roast': 8, 'Espresso': 9.5})
        self.V = V
    ...
    def b16_utility(self):
        ...
        arrow = Arrow(Y.line.get_left() + DOWN * 1.5, Y.line.get_right() + DOWN * 1.5, color=MUTED, buff=0, stroke_width=2, max_tip_length_to_length_ratio=0.03)
        ...
        self.play(at)
        self.wait()
        self.play(*W.set_values({'Espresso': 3, 'Tea': 9.5}), run_time=1.5)
        self.wait()
        W.freeze()
    ...
    def b33_grocery(self):
        ...
        q = Tex('What can a dollar get?').scale(0.9).move_to(UP * 2.2)
        self.play(FadeIn(q), Create(V.line))
    ...
    def b42_what_is_micro(self):
        ...
        clauses = [
            ("there's a deep fundamental {{reason}} why it pays to {{coordinate}} with each other,", ['reason', 'coordinate']),
    ...
    def b44_six_parts(self):
        ...
        for i, (p, name, sub) in enumerate(parts):
            row = VGroup(Tex(f'Part {p}.').set_color(TITLE), Tex(name).set_color(DEFINITION), Tex(sub)).scale(0.85)
