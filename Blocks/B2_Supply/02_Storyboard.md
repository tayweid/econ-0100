...
- In that same 0.8-second focus move, widen the selected bar to three times its resting width as the surrounding graph fades out. The orange horizontal MC line fades in and widens with the bar. Anchor the bar at its original left edge inside the graph and keep its original baseline and vertical scale. Keep the flat cost height at $2+(Q_{left}+Q_{right})/2$ throughout; only the width changes when shrinking back. Midpoint bars preserve the area under this linear supply curve without requiring a calculation on screen.
