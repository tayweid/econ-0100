...
and `1.e.exchanged` after the shortage count,
...
and waiting option remain separate at `1.f.match` and `1.f.wait`. The seller
now mirrors them at `1.h.match` and `1.h.wait`; all earlier 67 IDs retain their
relative order. `1.e.exchanged` identifies the 20 actual exchanges. These pauses supersede compressed pacing and earlier
...
- Put narration, principle, and definition footers at the centered bottom,
  ...
  In `1.d`, `1.e.willing`, `1.e`, and `1.e.exchanged`, use the title “A low price: $3” and retain
  the gold bottom question “Who trades?” at scale 0.7443 and margin 0.05. Fade
  the question when the `1.j` exercise enters. At `1.g`, use “A high price: $6”
...
2. Title: “A low price: $3.” Show “Who trades?” in gold at the bottom, scale
   ...
   and question through `1.e.willing`, `1.e`, and `1.e.exchanged`.
...
1. After the willingness hold, let each ground segment follow its person as the first 20 buyers and sellers move
   ...
   20 trades, which receive attached labels at the following hold.
...
## 1.e.exchanged · Identify the actual exchanges

1. After `1.e`, add “20 exchanged” in FOCUS yellow beside the matched pairs on
   the plaza and over the traded 0–20 portion of the demand graph. The graph
   label may wrap to two lines. Use the same measurement style as the nearby
   shortage readouts; the existing units establish that 20 means 20,000 lb.
2. Pause at `1.e.exchanged`. Retain the title “A low price: $3” and gold bottom
   “Who trades?” without adding another title or footer. Keep the shortage
   readouts unchanged. Fade both exchanged labels as the exercise enters.

## 1.j · Exercise B3 Q2 — before resolving equilibrium

1. After `1.e.exchanged`, fade its two quantity-exchanged labels and the bottom
   “Who trades?” question and replace the market with an
...
1. Return both sides to the outer rim and hide plaza checks, Xs, trades, and
   ...
   Title: “A high price: $6.” Show “Who trades?” in gold at the standard bottom
   position, scale 0.7443 and margin 0.05. Hold before revealing the plaza
   response and actual matching, retaining the question during matching.
...
1. Retain teal “30” and orange “80” at their graph quantity positions. Reveal
   ...
   Keep the title “A high price: $6.” Fade the “Who trades?” question as the
   gold caption “A seller who's left out offers a buyer less than $6.” appears
   at the standard bottom position (scale 0.7443, margin 0.05). Keep it through
   the zoom and decision steps. Keep the grey plaza units until the zoom;
   quantity-gap labels stay attached. Highlight the unserved seller and proposed
   cut, then pause at `1.h.offer` before zooming.
2. Inspect the deciding seller (internally seller 40) with buyer 30 in the
   familiar head-on view, retaining the existing “A high price: $6” context title
   ...
   Pause on this existing match at `1.h.match`.
3. Reveal “Keep $6: gain $0” on the left at world height 3.65, below the title
   and level with the right offer callout. Its short arrow points up to the
   current solid price. Pause at `1.h.wait`. Then reveal “Ask $5.75: gain
   $1.75/lb” with a short arrow to the proposed dashed price spanning seller
   and buyer. Keep both arrows clear of the bars. Retain the gold seller-offer
   caption at the existing `1.h` decision stop. On advance, fade that caption,
   move the buyer's MB bar/label to the new pair, and solidify the
...
1. Place Q1 immediately after `4.c`, completing the price controls and DWL
   ...
   **$(Q_d = 14;\ Q_s = 6)$** inline with the maximum legal price sentence.
   Put **Plot demand, supply, and the ceiling; shade CS, PS, and DWL.** on
   its own unnumbered setup line before a). Questions run from a) through d).
...
1. Use the sheet's 9-galleon floor and original prompts: quantity exchanged,
   ...
   **$(Q_d = 6;\ Q_s = 14)$** inline with the minimum legal price sentence.
   Put **Plot demand, supply, and the floor; shade CS, PS, and DWL.** on
   its own unnumbered setup line before a).
...
- The earlier 64-hold revision completed a full construction with animations skipped:
  ...
  review of every animation frame. The earlier 66-hold revision passed a focused
  ...
  The earlier 67-hold revision passed a focused construction check through the
  exercise: the new labels show 20 actual exchanges, the graph label fits the
  0–20 interval, and both labels clear on advance. The rendered hold was inspected;
  the previous 66 pause IDs retain their order.
- The 69-hold seller revision passed a focused construction check of the offer,
  match, wait, decision, and acceptance stages. The rendered decision was
  inspected: the left callout clears the title, both choice labels share a
  height, and the gold seller-offer footer persists until acceptance.
