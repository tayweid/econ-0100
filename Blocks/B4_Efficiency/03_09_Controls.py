...
class B4Controls(ThreeDScene):
    ...
    def construct(self):
        ...
        buyer_label = fixed(Tex('Buyers: highest MB first', color=DEMAND).scale(0.56).move_to([-3.3, 2.65, 0]))
        seller_label = fixed(Tex('Sellers: lowest MC first', color=SUPPLY).scale(0.56).move_to([-3.3, -2.55, 0]))
        units = fixed(Tex(r'One person = 1,000 lb', color=CAPTION).scale(0.50).move_to([-3.3, -2.90, 0]))
        ...
        ag_ring = Circle(radius=0.23, color=FOCUS, stroke_width=3).move_to(buyer_positions[24])
        ...
        andrew_ring = Circle(radius=0.23, color=FOCUS, stroke_width=3).move_to(seller_positions[39])
