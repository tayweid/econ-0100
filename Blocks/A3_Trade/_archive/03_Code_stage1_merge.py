...
#
# Episode A3 | Trade — STAGE 1 of the port: every scene of the old file
# (animation_0..animation_5, both drafts, and animation_old) carried into
# one flat construct(), verbatim and in file order, running on the OLD
# numbers via local shims. No pauses yet; waits kept. The storyboard and
# design pass come next. 03_Code_A4.py (absorbed old A4) is untouched.
...
# Stage-1 shims: the pre-flip Part A functions of record. Video.py now carries
# the carrots-first set (and PPF_Coop instead of PPF_Guild), so these locals
# shadow the star-import and every scene below renders exactly as it
# originally did. The A3 design pass replaces them with the new numbers.
def PPF_Molly(s):
    return 5 - s/9
...
class EpisodeA3(Scene):
    """Episode A3 | Trade — stage-1 consolidation (see header)."""

    def construct(self):

        # ---- animation_0 | Intro Sequence --------------------------------

        bumper(self, 'A', 3)

        # -- section bridge: clear the previous scene, reset the camera --
        for _m in [*self.mobjects]:
            if not isinstance(_m, VMobject):
                self.remove(_m)          # camera frames are bare Mobjects
        FadeAll(self)
        self.camera.frame.move_to(ORIGIN).set(width=FRAME_W)

        # ---- animation_1 | Last Time... ----------------------------------
        ...
        # ---- animation_2 | Make the Op Cost Table ------------------------
        ...
        # ---- animation_3 | Setup The Questions ---------------------------
        ...
        # ---- animation_4 | Trade Setup -----------------------------------

        # -- re-init of the module-level axes (each old scene saw these
        #    freshly imported in its own process) --
        PPF_axis = style_axes(            
            x_range=[0, 65, 5],
            x_length = 7,
            ticks=True,
        ...
        exchange_carrot = Tex('{{3}} {{C}}').set_color(CARROTS).scale(0.8).next_to(y_ex_p,RIGHT,buff=1/4)  # set_value dropped: Tex has none (original bug)
        exchange_spinach = Tex('{{27}} {{S}}').set_color(SPINACH).scale(0.8).next_to(x_ex_p,UP,buff=1/4)  # set_value dropped: Tex has none (original bug)
        ...
        exchange_carrot = Tex('{{2}} {{C}}').set_color(CARROTS).scale(0.8).next_to(y_ex_p,RIGHT,buff=1/4)  # set_value dropped: Tex has none (original bug)
        exchange_spinach = Tex('{{18}} {{S}}').set_color(SPINACH).scale(0.8).next_to(x_ex_p,UP,buff=1/4)  # set_value dropped: Tex has none (original bug)
...
# shadowed duplicate, kept for the rewrite
        ...
        # STAGE-1 NOTE: the play below was unrunnable in the original file too —
        # `dotm` / `dota` are never defined in this scene (draft tail, marked
        # "in progress" just below). Kept verbatim, disabled so the episode runs:
        # self.play(
        #     FadeIn(dotm, dota)
        # )
        ...
        # ---- animation_old | Trade With Another (unused) -----------------
